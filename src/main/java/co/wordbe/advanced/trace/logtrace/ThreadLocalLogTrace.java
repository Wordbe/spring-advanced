...
public class ThreadLocalLogTrace implements LogTrace {
    ...
    private void complete(TraceStatus status, Exception e) {
        ...
        if (e == null) {
            ...
            log.info("[{}] {}{} time={}ms ex={}", traceId.getId(), addSpace(EX_PREFIX, traceId.getLevel()), status.getMessage(), elapsedTimeMs, e.toString());
        }
    ...
    private void releaseTraceId() {
        ...
        if (traceId.isFirstLevel()) {
            traceIdHolder.remove(); // ThreadLocal 에 저장된 값을 제거한다.
        } else {
